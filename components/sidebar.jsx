...
const Sidebar = () => {
  ...
  const loadPendingCount = async () => {
    try {
      ...
      if (!auth.currentUser) {
        window.devLog('Skipping pending count load: Firebase Auth not ready');
        return;
      }

      // 🔒 Sadece kendi şirketinin önerilerini say
      const userCompany = user?.company || 'BLUEMINT';
      ...
      window.devLog('Pending suggestions count:', snapshot.size);
    } catch (e) {
