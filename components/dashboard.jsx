...
const Dashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const companyId = shouldFilterByCompany
        ? (currentUser.isSuperAdmin ? selectedCompany : currentUser.companyId)
        : null;

      let questionsQuery = window.supabase
        .from('questions')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(DASHBOARD_QUERY_LIMIT);

      let sessionsQuery = window.supabase
        .from('quiz_sessions')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(DASHBOARD_QUERY_LIMIT);

      let resultsQuery = window.supabase
        .from('results')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(DASHBOARD_QUERY_LIMIT);
