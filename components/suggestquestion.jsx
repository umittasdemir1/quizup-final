...
const SuggestQuestion = () => {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      window.devLog('Waiting for Firebase...');
      await waitFirebase();
      const { db, collection, addDoc, serverTimestamp } = window.firebase;
      window.devLog('Firebase ready');

      // 🆕 Otomatik şirket bilgisi ekle
      ...
      const suggestion = {
        ...
        company: userCompany, // 🆕 Şirket bilgisi
      ...
      window.devLog('Attempting to save to suggestedQuestions...');

      const docRef = await addDoc(collection(db, 'suggestedQuestions'), suggestion);

      window.devLog('SUCCESS! Document ID:', docRef.id);
