...
const Manager = () => {
  ...
  // Supabase company UUID için yardımcı
  ...
  // 📦 Load Question Packages - polling listener
  useEffect(() => {
    let unsubscribe;
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!companyId) {
        toast('Oturum oluşturmak için bir şirket seçin', 'error');
        return;
      }

      const creatorPin = currentUser?.applicationPin && /^\d{4}$/.test(currentUser.applicationPin)
