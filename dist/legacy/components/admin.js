  ...
  const Admin = () => {
    ...
    const uploadQuestionImage = async (file) => {
      ...
      try {
        ...
        toast("G\xF6rsel y\xFCklenirken hata olu\u015Ftu: " + e.message, "error");
      } finally {
        setUploading(false);
      }
    };
    ...
    const handleEdit = (q) => {
      ...
      setForm({
        ...
        isActive: (_a = q.isActive) != null ? _a : true,
    ...
    const handleSave = async () => {
      ...
      if (!Number.isFinite(orderNumber) || orderNumber < 1) {
        setErrors((prev) => ({ ...prev, orderNumber: "Ge\xE7erli bir numara giriniz" }));
        toast("Ge\xE7erli bir numara giriniz", "error");
        return;
      }
      const existingNumbers = questions.filter((q) => q.id !== editId).map((q, index) => getDisplayOrder(q, index));
      ...
      try {
        ...
        const validatedOptions = form.type === "mcq" ? form.options.filter((o) => o.trim()).map((o) => window.validateText ? window.validateText(o.trim()) : o.trim()) : [];
        const data = {
        ...
        if (editId) {
          ...
          toast("Soru g\xFCncellendi", "success");
        } else {
          const companyId = getDbCompanyId();
          if (!companyId) {
            toast("Soru eklemek i\xE7in bir \u015Firket se\xE7in", "error");
            setSaving(false);
            return;
          }
    ...
    if (loading) return /* @__PURE__ */ React.createElement(Page, { title: "Soru Havuzu" }, /* @__PURE__ */ React.createElement(LoadingSpinner, { text: "Sorular y\xFCkleniyor..." }));
    return /* @__PURE__ */ React.createElement(
      Page,
      {
        title: "Soru Havuzu",
        subtitle: `Toplam ${questions.length} soru`,
        extra: !showForm && /* @__PURE__ */ React.createElement("button", { className: "btn btn-primary", onClick: startCreate }, "+ Yeni Soru")
      },
