(() => {
  const LOCATION_CONSENT_KEY = "quizup:location:thirdPartyConsent";
  const NOMINATIM_MIN_INTERVAL_MS = 1100;
  const reverseGeocodeCache = /* @__PURE__ */ new Map();
  let lastNominatimRequestAt = 0;
  const requestLocationConsent = () => {
    try {
      const stored = localStorage.getItem(LOCATION_CONSENT_KEY);
      if (stored === "granted") return true;
      if (stored === "denied") return false;
    } catch {
    }
    const accepted = window.confirm(
      "Konumunuz quiz sonucuna eklenecek. \u015Eehir bilgisini bulmak i\xE7in koordinatlar OpenStreetMap Nominatim servisine g\xF6nderilebilir. Devam edilsin mi?"
    );
    try {
      localStorage.setItem(LOCATION_CONSENT_KEY, accepted ? "granted" : "denied");
    } catch {
    }
    return accepted;
  };
  const waitForNominatimSlot = async () => {
    const elapsed = Date.now() - lastNominatimRequestAt;
    if (elapsed < NOMINATIM_MIN_INTERVAL_MS) {
      await new Promise((resolve) => setTimeout(resolve, NOMINATIM_MIN_INTERVAL_MS - elapsed));
    }
    lastNominatimRequestAt = Date.now();
  };
  const reverseGeocode = async (lat, lng) => {
    const cacheKey = `${lat.toFixed(4)},${lng.toFixed(4)}`;
    if (reverseGeocodeCache.has(cacheKey)) {
      return reverseGeocodeCache.get(cacheKey);
    }
    await waitForNominatimSlot();
    const url = new URL("https://nominatim.openstreetmap.org/reverse");
    url.searchParams.set("lat", String(lat));
    url.searchParams.set("lon", String(lng));
    url.searchParams.set("format", "json");
    url.searchParams.set("accept-language", "tr");
    const response = await fetch(url.toString());
    if (!response.ok) {
      throw new Error(`Nominatim request failed: ${response.status}`);
    }
    const data = await response.json();
    reverseGeocodeCache.set(cacheKey, data);
    return data;
  };
  const getLocationFromIP = async () => {
    try {
      const response = await fetch("https://ipapi.co/json/");
      const data = await response.json();
      return {
        city: data.city || "Bilinmiyor",
        region: data.region || "",
        country: data.country_name || "Turkey",
        countryCode: data.country_code || "TR",
        lat: data.latitude || 39.9334,
        lng: data.longitude || 32.8597,
        ip: data.ip || "",
        source: "ip"
      };
    } catch (error) {
      window.devError("IP location error:", error);
      return {
        city: "Bilinmiyor",
        region: "",
        country: "Turkey",
        countryCode: "TR",
        lat: 39.9334,
        lng: 32.8597,
        ip: "",
        source: "default"
      };
    }
  };
  const getLocationFromBrowser = () => {
    return new Promise((resolve, reject) => {
      if (!navigator.geolocation) {
        reject(new Error("Geolocation not supported"));
        return;
      }
      if (!requestLocationConsent()) {
        reject(new Error("Location consent denied"));
        return;
      }
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          var _a, _b, _c, _d, _e, _f, _g;
          const lat = position.coords.latitude;
          const lng = position.coords.longitude;
          try {
            const data = await reverseGeocode(lat, lng);
            resolve({
              city: ((_a = data.address) == null ? void 0 : _a.city) || ((_b = data.address) == null ? void 0 : _b.town) || ((_c = data.address) == null ? void 0 : _c.county) || "Bilinmiyor",
              region: ((_d = data.address) == null ? void 0 : _d.state) || "",
              country: ((_e = data.address) == null ? void 0 : _e.country) || "Turkey",
              countryCode: ((_g = (_f = data.address) == null ? void 0 : _f.country_code) == null ? void 0 : _g.toUpperCase()) || "TR",
              lat,
              lng,
              accuracy: position.coords.accuracy,
              source: "gps"
            });
          } catch (error) {
            resolve({
              city: "Bilinmiyor",
              region: "",
              country: "Turkey",
              countryCode: "TR",
              lat,
              lng,
              accuracy: position.coords.accuracy,
              source: "gps"
            });
          }
        },
        (error) => {
          window.devError("Geolocation error:", error);
          reject(error);
        },
        {
          enableHighAccuracy: true,
          // Daha hassas konum
          timeout: 15e3,
          // 15 saniye timeout
          maximumAge: 6e4
          // 1 dakika cache
        }
      );
    });
  };
  const getLocation = async () => {
    try {
      const location = await getLocationFromBrowser();
      window.devLog("Location obtained from GPS:", location);
      return location;
    } catch (error) {
      window.devError("GPS location failed:", error);
      return {
        city: "Konum al\u0131namad\u0131",
        region: "",
        country: "Turkey",
        countryCode: "TR",
        lat: null,
        lng: null,
        accuracy: null,
        source: "unavailable",
        error: error.message
      };
    }
  };
  window.locationUtils = {
    getLocation,
    getLocationFromIP,
    getLocationFromBrowser
  };
})();
