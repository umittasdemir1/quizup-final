-- Keep the shared post-answer result phase to three seconds.
-- Restore participant XP in the live UI without changing score rules or RLS.
...
begin
  ...
  if r.session_id is null then
    ...
    select jsonb_agg(jsonb_build_object('id',q.id,'text',q.question_text,'type',q.type,'options',q.options,
      'correctAnswer',q.correct_answer,'answerExplanation',q.answer_explanation,'difficulty',q.difficulty,'category',q.category,
  ...
  loop
    ...
    if r.phase = 'lobby' and s.session_mode = 'open' then
      ...
      r.phase := 'reveal'; r.deadline := r.deadline + interval '3 seconds';
  ...
  if r.phase='question' and (active_count=0 or answered_count=active_count) then
    r.phase := 'reveal'; r.deadline := n + interval '3 seconds';
  end if;
  ...
  -- Recalculate only this participant's closed questions, using the same
  -- recorded server time and formula as final scoring. Polling is idempotent.
  if player.id is not null then
    for i in 0..total-1 loop
      if i < r.question_index or r.phase in ('reveal','finished') then
        q := r.questions->i; a := player.answers->i::text;
        if q->>'type'='mcq' and coalesce(a->>'value'=q->>'correctAnswer',false) then
          base := case q->>'difficulty' when 'easy' then 100 when 'hard' then 300 else 200 end;
          used := coalesce((a->>'timeUsed')::numeric,60);
          live_xp := live_xp + round(base*(1-0.5*used/60));
        end if;
      end if;
    end loop;
  end if;
  ...
  question_json := case when r.phase in ('question','reveal') and (moderator or player.id is not null) then
    case when r.phase='reveal' or moderator then q-'answerExplanation' else q-'correctAnswer'-'answerExplanation' end else null end;
  return jsonb_build_object('serverNow',clock_timestamp(),'mode',s.session_mode,'phase',r.phase,'questionIndex',r.question_index,
    ...
    'liveXp',case when player.id is not null then live_xp else null end,
    'answerFeedback',case when s.session_mode='duel' and not moderator and player.active
      and r.phase in ('question','reveal') and player.answers ? r.question_index::text
      and length(trim(coalesce(q->>'answerExplanation',''))) > 0
      then jsonb_build_object('text',q->>'answerExplanation','isCorrect',case when q->>'type'='mcq'
        then player.answers->r.question_index::text->>'value'=q->>'correctAnswer' else null end)
      else null end,
