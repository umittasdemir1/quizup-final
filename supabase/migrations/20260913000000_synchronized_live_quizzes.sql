-- Shared server deadlines and locked transitions. Guest capabilities are random UUIDs;
-- only their hashes are stored. These tables are accessible through RPCs only.
create table public.live_quiz_rooms (
...
create table public.live_quiz_players (
  ...
  session_id uuid not null references public.live_quiz_rooms(session_id) on delete cascade,
...
revoke all on public.live_quiz_rooms, public.live_quiz_players from anon, authenticated;
...
begin
  ...
  if profile.id is null or not (coalesce(profile.is_super_admin,false) or
  ...
  if p_mode not in ('open','duel') then raise exception 'Geçersiz mod'; end if;
  if p_mode = 'duel' then
