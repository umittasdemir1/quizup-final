-- Optional explanation; existing questions remain compatible with all modes.
alter table public.questions add column answer_explanation text not null default ''
  check (char_length(answer_explanation) <= 2000);

-- Snapshot explanations with the question; expose feedback only to the participant
-- whose answer is already recorded. Existing RLS, auth and CORS remain unchanged.
