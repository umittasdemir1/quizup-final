...
const useAnimatedPlaceholder = () => {
  ...
  // Load placeholder words from Firebase based on user's company
  useEffect(() => {
    (async () => {
      try {
        await waitFirebase();
        ...
        try {
          ...
          console.warn('Could not get user company for placeholder:', err);
        }

        if (userCompany) {
          // Load company-specific branding
          const brandingDoc = await getDoc(doc(db, 'branding', userCompany));
