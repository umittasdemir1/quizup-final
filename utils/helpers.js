...
const createToast = (msg, kind = 'info', duration = 3000) => {
  ...
  progressEl.style.color = kind === 'error' ? '#dc2626' :
    kind === 'success' ? '#5EC5B6' :
    kind === 'warning' ? '#FF6B4A' : '#4A90A4';

  toast.append(iconEl, messageEl, closeEl, progressEl);
...
const getCurrentUser = () => {
  if (window.__quizupAuthReady && window.__quizupAuthUser !== undefined) {
...
const sendSessionHeartbeat = async (force = false) => {
  ...
  try {
    await window.db?.updateSessionHeartbeat?.(
...
const registerActiveSession = async (userId) => {
  ...
  const registrationPromise = (async () => {
    ...
    try {
      await window.db?.registerSession?.(userId, sessionId, {
        device,
        createdAtMs: now,
        lastActiveAtMs: now
      });
...
// Get company identifiers for legacy company-name based queries
...
const requireAuth = (requiredRole = null) => {
  ...
  window.__quizupAuthReadyPromise
    ?.then(verifyAccess)
    .catch((err) => {
  ...
  try {
    if (window.signOutSupabase) {
      if (currentUser?.uid && sessionId) {
        await window.db?.removeSession?.(currentUser.uid, sessionId);
      }
